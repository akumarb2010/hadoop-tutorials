package tutorial.hadoop.hive.test;
...
public class HadoopHiveUDFKellyTest {

	private static String driverName = "org.apache.hadoop.hive.jdbc.HiveDriver";

	private static String tableName = "kelly30";

	private static String partitionedTableName = "kelly1";
	
	private static String demoTableName = "kellyDemoTable";

	private static String demoPartitionedTableName = "kellyPartitionDemo1";
	
	private String hiveDataFilePath = "/home/kelly1/Work/input/hiveTable.log";
	
	private String hivePartitionDataFilePath = "/home/kelly1/Work/input/hivePartitionTable.log";

	private String dataFilePath = "/home/kelly1/Work/input/k1.log";
...
public static String hiveURI = "jdbc:hive://localhost:10000/myderby1";
	...
	public HadoopHiveUDFKellyTest(String name) {
	...
	protected Statement setUpPartitionTable(Connection con) throws Exception {
		...
		/*res = stmt.executeQuery("load data local inpath '"
				+ hiveDataFilePath.toString() + "' into table " + demoPartitionedTableName);*/
		
		res = stmt.executeQuery("load data local inpath '"
				+ hivePartitionDataFilePath.toString() + "' into table "
				+ demoPartitionedTableName + " PARTITION (dt='67889')");
		return stmt;
	}

	protected ResultSet executeQuery(Statement stmt, String query)
			throws Exception {
	...
	public static void main(String[] args) {
		try {
			...
			HadoopHiveUDFKellyTest test = new HadoopHiveUDFKellyTest("Hive");
//			Thread.sleep(1000);
...
//			Statement stmt = con.createStatement();
	...
	/*		System.out.println("Setting up sample table....");
			Statement stmt = test.setUpPartitionTable(con);
			System.out.println("Executing sammple partitioned query....");
			ResultSet set = test.executeQuery(stmt, "select max(key) from "+demoPartitionedTableName);
			System.out.println("Printing results...");
			printResultSet(set);*/
			// bucketization based query
//			System.out.println(" Printing from bucket....");
//			set = test.executeQuery(stmt, "SELECT * FROM "+demoPartitionedTableName+" TABLESAMPLE(2 OUT OF n BUCKETS)WHERE dt=’20120929’ ");
			// con.close();
		} catch (Exception e) {
			e.printStackTrace();
		}
