...
public class HadoopHiveUDFTest {
	...
	private String dataFilePath = "D:/Grid/testdata/k1.log";

	private String dataFilePath1 = "D:/Grid/testdata/table1.txt";

	private String dataFilePath2 = "D:/Grid/testdata/table2.txt";

	private boolean standAloneServer = true;

	public static String hiveURI = "jdbc:hive://AP-BGLR-LP0458:10000/default";
	
//	public static String hiveURI = "jdbc:hive://10.253.116.170:10000/default";
...
//		stmt.executeQuery("drop table " + tableName);
		...
		ResultSet res = stmt
				.executeQuery("create table xtable(id int,name string)ROW FORMAT DELIMITED FIELDS TERMINATED BY ',' STORED AS TEXTFILE");

		res = stmt
				.executeQuery("create table ytable(name string,country string)ROW FORMAT DELIMITED FIELDS TERMINATED BY ',' STORED AS TEXTFILE");
	...
	protected void addUDFJar(Statement stmt) throws Exception {
		String addJarquery = "add jar D:/Working/Workspace/RaptorWS/HadoopRaptor/HiveResearch/lib/aMovingAverage.jar";
		ResultSet res = stmt.executeQuery(addJarquery);
		while (res.next()) {
			System.out.println(res.getString(1));
		}
		String createTempFun = "create temporary function mavg as 'com.sungard.ats.hive.udf.UDFMovingAverage'";
	...
	protected static void printResultSet(ResultSet res) throws Exception {
		if (res == null)
			return;
		while (res.next()) {
			System.out.println(res.getDouble(1) + "," + res.getDouble(2) + ","
					+ res.getString(3));
		}
	}
	...
	public static void main(String[] args) {
		try {
			...
			/*
			 * Statement stmt = con.createStatement();
			 * set = test.executeQuery(stmt, "select COUNT(1) from UDFTest");
